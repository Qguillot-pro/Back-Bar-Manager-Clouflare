...
const AdminLogbook: React.FC<AdminLogbookProps> = ({ currentUser, onSync, onClose, isEmbedded = false }) => {
  ...
  if (!isUnlocked) {
      return (
          <div className={`${isEmbedded ? 'p-12' : 'fixed inset-0 z-[2000] bg-slate-900 flex items-center justify-center p-4'}`}>
              <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center space-y-6 mx-auto">
  ...
  return (
    <div className={`${isEmbedded ? 'bg-white flex flex-col min-h-[600px]' : 'fixed inset-0 z-[2000] bg-white flex flex-col'}`}>
        <div className="bg-slate-900 text-white p-4 flex justify-between items-center shadow-lg shrink-0">
            ...
            <div className="flex gap-2">
                ...
                {!isEmbedded && <button onClick={onClose} className="px-4 py-2 bg-rose-500 hover:bg-rose-600 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ml-4">Fermer</button>}
            </div>
        </div>
