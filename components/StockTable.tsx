...
const StockTable: React.FC<StockTableProps> = ({ items, storages, stockLevels, setStockLevels, priorities, onUpdateStock, onAdjustTransaction, consignes = [], currentUser, onSync, canEdit = true }) => {
  ...
  const handleManualEdit = (itemId: string, storageId: string, val: number) => {
      ...
      onUpdateStock(itemId, storageId, Math.round(val * 1000) / 1000, "Régulation", true);
  };
                                    ...
                                    return (
                                        <td key={s.id} className="p-4 border-r text-center">
                                            <div className="flex justify-center items-center gap-2">
                                                {canEdit && <button onClick={() => onAdjustTransaction?.(item.id, s.id, -1, true)} className="w-7 h-7 rounded-lg bg-slate-100 text-slate-400 hover:bg-rose-500 hover:text-white font-black transition-all">-</button>}
                                                <EditableNumberCell 
                                                    value={qty}
                                                ...
                                                />
                                                {canEdit && <button onClick={() => onAdjustTransaction?.(item.id, s.id, 1, true)} className="w-7 h-7 rounded-lg bg-slate-100 text-slate-400 hover:bg-emerald-500 hover:text-white font-black transition-all">+</button>}
                                            </div>
                                        </td>
                                    );
                                })}
                                      ...
                                      <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                                          ...
                                          <td className="p-6 text-center">
                                              <div className="flex justify-center gap-2">
                                                  {canEdit && <button onClick={() => onAdjustTransaction?.(selectedProductId, s.id, -1, true)} className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 font-black hover:bg-rose-500 hover:text-white transition-all shadow-sm">-</button>}
                                                  {canEdit && <button onClick={() => onAdjustTransaction?.(selectedProductId, s.id, 1, true)} className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 font-black hover:bg-emerald-500 hover:text-white transition-all shadow-sm">+</button>}
                                              </div>
                                          </td>
                                      </tr>
                                  );
                              ...
                              return (
                                  <tr key={item.id} className={`hover:bg-slate-50 transition-colors ${isUnauthorized ? 'bg-rose-50/30' : ''}`}>
                                      ...
                                      {!isReorderMode && (
                                          <td className="p-6 text-center">
                                              <div className="flex justify-center gap-2">
                                                  {canEdit && <button onClick={() => onAdjustTransaction?.(item.id, selectedStorageId, -1, true)} className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 font-black hover:bg-rose-500 hover:text-white transition-all shadow-sm">-</button>}
                                                  {canEdit && <button onClick={() => onAdjustTransaction?.(item.id, selectedStorageId, 1, true)} className="w-10 h-10 rounded-xl bg-slate-100 text-slate-600 font-black hover:bg-emerald-500 hover:text-white transition-all shadow-sm">+</button>}
                                              </div>
                                          </td>
                                      )}
                                  </tr>
