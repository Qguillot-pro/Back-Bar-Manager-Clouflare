...
const StorageOptimization: React.FC<StorageOptimizationProps> = ({ items, storages, stockLevels, consignes, transactions, formats, orders, onAction, onSync }) => {
    ...
    const analysis = useMemo(() => {
        ...
        sortedItems.forEach(item => {
            ...
            itemConsignes.forEach(consigne => {
                ...
                const itemStock = stockLevels.find(l => l.itemId === item.id && l.storageId === consigne.storageId)?.currentQuantity || 0;

                if (diffDays >= 7 && itemStock > 0) {
    ...
    const handlePostpone = (itemId: string) => {
        ...
        onSync('SAVE_ITEM', { ...item, analysisPostponedUntil: nextWeek.toISOString() });
    };

    const handleSetNotFollowed = (itemId: string, val: boolean) => {
        const item = items.find(i => i.id === itemId);
        if (!item) return;
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-8 pb-20">
            <header className="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl border border-white/10 flex flex-col md:flex-row justify-between items-center gap-6">
                <div className="flex-1">
                    <div className="flex items-center gap-4">
                        ...
                        <button 
                            onClick={() => setShowNotFollowedModal(true)}
                            className="bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white p-2 rounded-xl transition-all border border-slate-700"
                            title="Gérer les produits non suivis"
                        >
                            <Settings2 className="w-5 h-5" />
                        </button>
                    </div>
                    <p className="text-slate-400 text-xs font-bold mt-1 uppercase tracking-widest leading-relaxed">
                        Optimisez votre espace de stockage limité • {items.length} produits suivis
                    </p>
                </div>
                            ...
                            {analysis.tension.length === 0 ? (
                                ...
                                analysis.tension.map((a, idx) => (
                                    <div key={idx} className="p-4 hover:bg-slate-50 transition-colors group">
                                        <div className="flex justify-between items-start mb-2 text-wrap pr-16 relative">
                                            <div>
                                                <p className="font-black text-slate-900 text-sm leading-tight">{a.item.name}</p>
            ...
            {/* MODAL GESTION NON SUIVIS */}
            {showNotFollowedModal && (
                <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md animate-in fade-in">
                    <div className="bg-white rounded-[2.5rem] p-8 max-w-2xl w-full shadow-2xl border border-slate-200 flex flex-col max-h-[80vh]">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-xl font-black text-slate-900 uppercase">Produits Non Suivis</h3>
                            <button onClick={() => setShowNotFollowedModal(false)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
                                <Trash2 className="w-5 h-5" />
                            </button>
                        </div>
                        
                        <div className="overflow-y-auto flex-1 space-y-2 pr-2">
                            {notFollowedItems.length === 0 ? (
                                <div className="py-12 text-center text-slate-400 italic font-medium">
                                    Aucun produit n'est classé "Non Suivi".
                                </div>
                            ) : (
                                notFollowedItems.map(item => (
                                    <div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                                        ...
                                        <button 
                                            onClick={() => handleSetNotFollowed(item.id, false)}
                                            className="px-4 py-2 bg-indigo-600 text-white rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2"
                                        >
                        ...
                        <div className="mt-8">
                            <button 
                                onClick={() => setShowNotFollowedModal(false)}
